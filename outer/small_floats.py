...
def replace_or_concat(s,r,v):
    if r in s:
        return s.replace(r,v)
    return s+v
...
class halffloat:
    ...
    def m_base_conv(self,dl=0,dh=1,base=10,full=True,prec=1):
        g = float(abs(self))
        ...
        #print('(',l,h,')','start:(',dl,dh,')')
        while not (l < dl < h and (l < dh < h or not full)):
            s = dh-dl
            d = min(max(0,int(base*(g-dl)/s)),base-1)
            if (not full):
                if d+1 < base and dl+s*(d+1)/base < h and not dl+s*d/base > l:
                    d += 1
            else:
            ...
            dl = dl+(d/base)*s
            dh = dl + s/base
            #print(d,'<-- (',dl,dh,') size:',s)
            yield d
    def num_str(self,full=False,base = '0123456789',prec=1):
        s = ['','-'][self.sign()]
        if self.is_inf():
            return s+'inf'
        if self.is_nan():
        ...
        h = len(base)**hi
        g = self.m_base_conv(0,h,len(base),full,prec)
        r = ''
        for v in g:
            hi -= 1
            if v != 0:
    ...
    def __repr__(self,*a):
        #return '½'+self.num_str()
        return replace_or_concat(self.num_str(*a),'.','.ͪ')
    def __str__(self,*a):
        return self.num_str(*a)
...
class minifloat(halffloat):
    ...
    def __repr__(self,*a):
        #return '¼'+self.num_str()
        return replace_or_concat(self.num_str(*a),'.','.ͫ')
